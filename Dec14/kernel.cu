...
__device__ void extract(int32_t *sorted_values, int32_t *res, uint8_t *modes)
{
   ...
   for ( int counter = 0; counter < TOTAL_MODES; counter++)
   {
         ...
         printf("mode in extract is %d\n",mode);

       //  printf("res_counter : %d\n",res[counter]);
       //  printf("sorted counter : %d\n",modes[counter]);
    ...
    */
    ...
    for(int mode =0;mode <35;mode++)
    {
        ...
        if(bsize == 4)
        {
           ...
           ay[ty][tx]  = abs(predSamplesY[ty][tx]  - y[row*width + col]);
           acr[ty][tx] = abs(predSamplesCr[ty][tx] - cr[row*width + col]);
           acb[ty][tx] = abs(predSamplesCb[ty][tx] - cb[row*width + col]);
           ...
           {  
              ...
              if(ty == 1)
              {
                  ...
                  cr_satd_shared[mode] = ((cr_satd_shared[mode] << 8) | mode) >> 1;
              }
              if(ty == 2)
              {
                  ...
                  cb_satd_shared[mode] = ((cb_satd_shared[mode] << 8) | mode) >> 1;
              }
          }      
    ...
    if ( 0 == ty && 0 == tx )
    {
       ...
       extract(ptr_y_satd_shared, res_y, y_modes);
