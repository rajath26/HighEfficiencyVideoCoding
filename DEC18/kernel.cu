...
__global__ void hevcPredictionKernel(uint8_t *y, uint8_t *cr, uint8_t *cb, int32_t *res_y, int32_t *res_cr, int32_t *res_cb, uint8_t *y_modes, uint8_t *cr_modes, uint8_t *cb_modes, int height, int width)
{
    ...
    */
    ...
    for(int mode =0;mode <35;mode++)
    {
        ...
        /*
        int offsetRef = abs((bsize * ipa[mode]) >> 5);
        ...
        */
        ...
        if(mode==PLANAR_MODE)
        {
            ...
            /*
            predSamplesY[ty][tx]=((bsize-1-ty)*selyy[tx]+(ty+1)*selxy[bsize]+(bsize-1-tx)*selxy[ty]+(tx+1)*selyy[bsize]+bsize)>>intLog; //TO_DO: Replace logarithmic with appropriate C function
            predSamplesCr[ty][tx]=((bsize-1-ty)*pycr[tx]+(ty+1)*pxcr[bsize]+(bsize-1-tx)*pxcr[ty]+(tx+1)*pycr[bsize]+bsize)>>intLog; //TO_DO: Replace logarithmic with appropriate C function
            predSamplesCb[ty][tx]=((bsize-1-ty)*pycb[tx]+(ty+1)*pxcb[bsize]+(bsize-1-tx)*pxcb[ty]+(tx+1)*pycb[bsize]+bsize)>>intLog; //TO_DO: Replace logarithmic with appropriate C function
            */
