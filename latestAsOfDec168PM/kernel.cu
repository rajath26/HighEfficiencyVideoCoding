...
__global__ void hevcPredictionKernel(uint8_t *y, uint8_t *cr, uint8_t *cb, int32_t *res_y, int32_t *res_cr, int32_t *res_cb, uint8_t *y_modes, uint8_t *cr_modes, uint8_t *cb_modes, int height, int width)
{
    ...
    */
    ...
    for(int mode =0;mode <35;mode++)
    {
        ...
        if(filterFlag==1)
        {
            if(bx ==var && by==var1 && tx==0 && ty==0 && mode >= 2)
                  printf("For bsize : %d For angular mode %d selected pf as neighbor\n",bsize,mode);
            selyy=&pf_yy[ONE];
            selxy=&pf_xy[ZERO];
        }
        else
        {
            ...
            if(bx ==var && by==var1 && tx==0 && ty==0 && mode >= 2)
                  printf("For bsize: %d For angular mode %d selected p as neighbor\n",bsize,mode);
            selyy=&p_yy[ONE];
            selxy=&p_xy[ZERO];
        }   
        ...
        */
        ...
        else if ( mode > DC_MODE && mode < ANGULAR_18 )
        {
            ...
            if ( bx == var && by == var1 && tx == 0 && ty == 0 && mode == 17 && bsize == 8 )
            {
                printf("\nFOR MODE %d SELYY\n", mode);
                for ( int i = -1 ; i < 2*bsize; i++)
                {
                    printf("\t%u", selyy[i]);
                }
                printf("\nFOR MODE %d SELXY\n", mode);
            ...
            if ( bx == var && by == var1 && 0 ==tx && 0 ==ty && mode == 17){
                printf("\nOFFSETVAL%d\n", offsetVal);
                printf("\nOFFSETREF%d\n", offsetRef);
            }
                ...
                if (ipa[mode] < 0)
                {
                    if ( ((bsize * ipa[mode]) >> 5) < -1 )
                    {
                        if ( 0 == tx && 0 == ty )
                        {
                            ...
                            /*
                            //DEBUG
                            if(bx==var && by==var1 && tx==0 && ty==0 && bsize==8){
                                for(int i=0;i<2*bsize;i++)
            ...
            */
            ...
            // Load iIdx and iFact
            iIdx1 = ( (tx + 1) * ipa[mode] ) >> 5;
            iFact1 = ( (tx + 1) * ipa[mode] ) & 31;
             
            __syncthreads();
            if ( iFact[ty][tx] != 0 )
            {
                predSamplesY[ty][tx] = ((32 - iFact[ty][tx]) * refY[ty + iIdx1 + 1] + iFact1 * refY[ty + iIdx1 + 2] + 16) >> 5;
                predSamplesCr[ty][tx] = ((32 - iFact[ty][tx]) * refCr[ty + iIdx1 + 1] + iFact1 * refCr[ty + iIdx1 + 2] + 16) >> 5;
                predSamplesCb[ty][tx] = ((32 - iFact[ty][tx]) * refCb[ty + iIdx1 + 1] + iFact[ty][tx] * refCb[ty + iIdx1 + 2] + 16) >> 5;
            }
            else
            {
                predSamplesY[ty][tx] = refY[ty + iIdx1 + 1];
                predSamplesCr[ty][tx] = refCr[ty + iIdx1 + 1];
                predSamplesCb[ty][tx] = refCb[ty + iIdx1 + 1];
            }
